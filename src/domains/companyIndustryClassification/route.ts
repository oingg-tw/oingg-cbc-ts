 ...
 *       自己、總機構統一編號=空白），取其行業代號（一列最多 4 組：主要+次要1/2/3，實測驗證過台積電
 *       同一列就有 3 組），逐組轉換成 tax_industry_classification 的 subclass 代碼格式後整批寫進
 *       company_industry_classifications（用 rank 欄位區分 0=主要、1/2/3=次要；每次重新 ingest 對
 *       該公司整批刪除重建，不逐筆比對更新）。
 *
 ...
 *     responses:
 *       200:
 *         description: 回填完成，回傳追蹤家數、成功比對數（家）、實際寫入的分類列數（含次要代碼）、稅籍檔裡找不到的家數、行業代號對不到分類表的家數。
 *       502:
